...
cpp_fused__softmax_0 = async_compile.cpp_pybinding(['const bfloat16*', 'float*', 'float*', 'bfloat16*'], '''
                       ...
                       bfloat16* out_ptr2)
{
    #pragma omp parallel num_threads(32)
    {
        ...
        {
            std::unique_ptr<float []> buf_local_buffer_data_0 = std::make_unique<float []>(1024L);
            float* local_buffer_data_0 = buf_local_buffer_data_0.get();
            #pragma omp for
            for(int64_t x0=static_cast<int64_t>(0L); x0<static_cast<int64_t>(49152L); x0+=static_cast<int64_t>(1L))
            {
                {
                    ...
                    {
                        auto tmp0 = at::vec::Vectorized<bfloat16>::loadu(in_ptr0 + static_cast<int64_t>(x1 + (1024L*x0)), 16);
                ...
                }
                {
                    ...
                    for(int64_t x1=static_cast<int64_t>(0L); x1<static_cast<int64_t>(1024L); x1+=static_cast<int64_t>(16L))
                    {
                        ...
                        auto tmp4 = tmp1 - tmp3;
                        auto tmp5 = tmp4.exp();
                        tmp5.store(local_buffer_data_0 + static_cast<int64_t>(x1));
                ...
                for(int64_t x1=static_cast<int64_t>(0L); x1<static_cast<int64_t>(1024L); x1+=static_cast<int64_t>(16L))
                {
                    auto tmp0 = at::vec::Vectorized<float>::loadu(local_buffer_data_0 + static_cast<int64_t>(x1), 16);
                    auto tmp1 = out_ptr1[static_cast<int64_t>(x0)];
                    auto tmp2 = at::vec::Vectorized<float>(tmp1);
                    auto tmp3 = tmp0 / tmp2;
                    auto tmp4 = at::vec::convert<bfloat16>(tmp3);
                    tmp4.store(out_ptr2 + static_cast<int64_t>(x1 + (1024L*x0)), 16);
                }
            }
        }
    }
