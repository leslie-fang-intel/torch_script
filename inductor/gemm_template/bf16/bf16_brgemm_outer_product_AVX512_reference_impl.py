                       ...
                       bfloat16* __restrict__  out_ptr1)
{
    ...
    auto loadc = [&](auto i) {
        if constexpr (accum) {
            ...
            vc[i] = _mm512_castsi512_ps(_mm512_slli_epi32(_mm512_cvtepu16_epi32(_mm512_castsi512_si256(
                        VectorizeBF16::loadu(out_ptr1 + row * ldc + col * VLEN, 16)
                    )),16));
        } else {
    ...
    auto compute = [&, COLS](auto i, int k) {
        ...
        if constexpr (col == 0) {
            if (alpha != 1) {
               va = _mm512_castsi512_ps(_mm512_slli_epi32(_mm512_cvtepu16_epi32(_mm512_castsi512_si256(
                        VectorizeBF16((in_ptr0[row * lda + k]) * alpha)
                    )),16));
            } else {
               va = _mm512_castsi512_ps(_mm512_slli_epi32(_mm512_cvtepu16_epi32(_mm512_castsi512_si256(
                        VectorizeBF16((in_ptr0[row * lda + k]))
                    )),16));                                                
            }
        }
        if constexpr (row == 0) {
            vb[col] = _mm512_castsi512_ps(_mm512_slli_epi32(_mm512_cvtepu16_epi32(_mm512_castsi512_si256(
                        VectorizeBF16::loadu(in_ptr1 + k * ldb + col * VLEN, 16)
                      )),16));
        }
