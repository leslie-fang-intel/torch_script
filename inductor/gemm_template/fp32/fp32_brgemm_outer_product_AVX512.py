...
# Outer Product
cpp_fused__softmax_0_outer_product = async_compile.cpp_pybinding(['const float*', 'const float*', 'float*'], '''
                       ...
                       float* __restrict__  out_ptr1)
{
    ...
    auto compute = [&, COLS](auto i, int k) {
        ...
        if constexpr (col == 0) {
            if (alpha != 1) {
               va = VectorizeF32((in_ptr0[row * lda + k]) * alpha);
            } else {
               va = VectorizeF32((in_ptr0[row * lda + k]));                                                  
            }
        }
        if constexpr (row == 0) {
            vb[col] = VectorizeF32::loadu(in_ptr1 + k * ldb + col * VLEN);
        }
