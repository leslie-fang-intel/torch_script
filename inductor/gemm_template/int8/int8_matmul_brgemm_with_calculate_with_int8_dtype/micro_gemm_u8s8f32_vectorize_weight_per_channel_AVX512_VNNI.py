                       ...
                       const int* __restrict__ b_zp                                        )
{
    ...
    signed char B_VNNI_Layout[K*BLOCK_N];
    int idx = 0;
    for (int i = 0; i < K; i+=4) {
    ...
    auto compute = [&, COLS](auto i, int k) {
        ...
        if constexpr (col == 0) {
            if (alpha != 1) {
               va = Vectorized((((int*)in_ptr0)[row * (lda/4) + k]) * alpha);
            } else {
               va = Vectorized((((int*)in_ptr0)[row * (lda/4) + k]));                                                  
            }
        }
        if constexpr (row == 0) {
            // vb[col] = at::vec::convert_to_int32(in_ptr1 + k * ldb + col * VLEN);
            vb[col] = Vectorized::loadu(((int*)B_VNNI_Layout) + k * ldb + col * VLEN);
        }
        constexpr int idx = row * COLS + col;
        // Do u8s8s32 fmadd with dpbusds
        vc[idx] = _mm512_dpbusds_epi32(vc[idx], va, vb[col]);
    };
