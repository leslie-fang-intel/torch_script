...
_frozen_param2 = None  # device(type='cpu') torch.float32 (48, 128) (128, 1) 7f56b3bef200
_frozen_param3 = None  # device(type='cpu') torch.float32 (1982689, 1) (1, 0) 7f56b3bef390
constant1 = None  # device(type='cpu') torch.float32 (1, 128, 48) (6144, 48, 1) 7f56b09c84b0


cpp_fused_mm_0 = async_compile.cpp_pybinding(['const float*', 'const float*', 'float*'], '''
#include "/localdisk/leslie/torch_ipex_gcc12/pytorch/torch/_inductor/codegen/cpp_prefix.h"
    ...
    for (int64_t m = 0; m < M; m += 8) {
        ...
        for (int64_t n = 0; n < N; n += 48) {
            if (block_m == 8) {
                ...
                switch (block_m) {
                case 7:
                    kernel_micro_gemm_kernel<7, 48, accum>(
                        A + m * lda,
                        B + n,
                        C + m * ldc + n,
                        K,
                    ...
                    break;
                case 6:
                    kernel_micro_gemm_kernel<6, 48, accum>(
                        A + m * lda,
                        B + n,
                        C + m * ldc + n,
                        K,
                    ...
                    break;
                case 5:
                    kernel_micro_gemm_kernel<5, 48, accum>(
                        A + m * lda,
                        B + n,
                        C + m * ldc + n,
                        K,
                    ...
                    break;
                case 4:
                    kernel_micro_gemm_kernel<4, 48, accum>(
                        A + m * lda,
                        B + n,
                        C + m * ldc + n,
                        K,
                    ...
                    break;
                case 3:
                    kernel_micro_gemm_kernel<3, 48, accum>(
                        A + m * lda,
                        B + n,
                        C + m * ldc + n,
                        K,
                    ...
                    break;
                case 2:
                    kernel_micro_gemm_kernel<2, 48, accum>(
                        A + m * lda,
                        B + n,
                        C + m * ldc + n,
                        K,
                    ...
                    break;
                case 1:
                    kernel_micro_gemm_kernel<1, 48, accum>(
                        A + m * lda,
                        B + n,
                        C + m * ldc + n,
                        K,
...
void kernel(const float* X, const float* W, float* Y)
{
    ...
    constexpr int64_t Nr = 48;
    constexpr int64_t Kr = 1;
    constexpr int64_t Nr_blocks = (N + Nr - 1) / Nr;
    constexpr int64_t Kr_blocks = (K + Kr - 1) / Kr;
    constexpr int64_t M = static_cast<int64_t>(32L);
                    ...
                    for (int64_t nci = nc; nci < nc_block_end; nci++) {
                        if (kc == k_block_start) {
                            kernel_micro_gemm<static_cast<bool>(false)>(
                                &(X[static_cast<int64_t>(k_start + (128L*m_start))]),
                                &(W[static_cast<int64_t>((48L*k_start) + (6144L*nci))]),
                                &(Y[static_cast<int64_t>(n_start + (48L*m_start) + (Nr*nci) + ((-1L)*Nr*nc))]),
                                static_cast<int64_t>(m_end + ((-1L)*m_start)),
                                static_cast<int64_t>(Nr),
                                static_cast<int64_t>(k_end + ((-1L)*k_start)),
                                static_cast<int64_t>(128L),
                                static_cast<int64_t>(48L),
                                static_cast<int64_t>(48L)
                            );
...
cpp_fused_silu_1 = async_compile.cpp_pybinding(['const float*', 'float*'], '''
#include "/localdisk/leslie/torch_ipex_gcc12/pytorch/torch/_inductor/codegen/cpp_prefix.h"
...
{
    {
        ...
        for(int64_t x0=static_cast<int64_t>(0L); x0<static_cast<int64_t>(32L); x0+=static_cast<int64_t>(1L))
        {
            ...
            {
                auto tmp0 = at::vec::Vectorized<float>::loadu(in_ptr0 + static_cast<int64_t>(x1 + (48L*x0)), static_cast<int64_t>(16));
                auto tmp1 = decltype(tmp0)(1)/(decltype(tmp0)(1) + tmp0.neg().exp());
                auto tmp2 = tmp0 * tmp1;
                tmp2.store(out_ptr0 + static_cast<int64_t>(x1 + (24L*x0)));
            }
            for(int64_t x1=static_cast<int64_t>(16L); x1<static_cast<int64_t>(24L); x1+=static_cast<int64_t>(8L))
            {
                auto tmp0 = at::vec::Vectorized<float>::loadu(in_ptr0 + static_cast<int64_t>(x1 + (48L*x0)), static_cast<int64_t>(8L));
                auto tmp1 = decltype(tmp0)(1)/(decltype(tmp0)(1) + tmp0.neg().exp());
                auto tmp2 = tmp0 * tmp1;
                tmp2.store(out_ptr0 + static_cast<int64_t>(x1 + (24L*x0)), static_cast<int64_t>(8L));
            }
        }
    }
}
...
def call(args):
    ...
    buf0 = empty_strided_cpu((32, 48), (48, 1), torch.float32)
    cpp_fused_mm_0(arg1_1, constant1, buf0)
    del arg1_1
    buf1 = empty_strided_cpu((32, 24), (24, 1), torch.float32)
    cpp_fused_silu_1(buf0, buf1)
    return (buf1, )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    global constant1
    constant1 = rand_strided((1, 128, 48), (6144, 48, 1), device='cpu', dtype=torch.float32)
    arg1_1 = rand_strided((32, 128), (128, 1), device='cpu', dtype=torch.float32)
