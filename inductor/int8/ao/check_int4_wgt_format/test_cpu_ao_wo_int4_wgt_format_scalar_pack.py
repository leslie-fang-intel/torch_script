...
ref_int4_gemm = async_compile.cpp_pybinding(['const bfloat16*', 'const unsigned char*', 'const bfloat16*', 'bfloat16*', 'long', 'long', 'long', 'long'], '''
#include "/localdisk/leslie/torch_inductor_community/pytorch/torch/_inductor/codegen/cpp_prefix.h"
#include <iostream>
#include <c10/util/Unroll.h>
    ...
    for (int m = 0; m < M; m += 1) {
        for (int n = 0; n < N; n += 1) {
            ...
            for (int k = 0; k < K; k += 1) {
                ...
                long idx = (n * K + k) / 2;
                long offset = 1 - (n * K + k) % 2;
                unsigned char val = in_ptr1[idx];
                int index = ((val & (0xF << (offset * 4))) >> (offset * 4));
                const auto b_val = lut[index] * scale + zero;
...
def torchao_GPTQ_int4():
    with torch.no_grad():
        ...
        true_res2 = torch.ops.aten._weight_int4pack_mm(
            x.to(torch.bfloat16),
            torch.ops.aten._convert_weight_to_int4pack(unpack_b, 8),
            256, # qGroupSize
