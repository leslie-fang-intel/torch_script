...
def run_model(model_name):
    ...
    class Mod(torch.nn.Module):
        def __init__(self, inplace_add=False, inplace_relu=False) -> None:
            ...
            self.relu1 = torch.nn.ReLU(inplace=inplace_relu)
        ...
        def forward(self, x):
            ...
            x1 = self.maxpool(self.relu1(self.conv(x)))
            return self.relu2(self.conv2(x1) + self.conv3(x1))
    ...
    with torch.no_grad():
        ...
        print(torch.allclose(ref_res, res, atol=1e-1, rtol=1e-1), flush=True)
