...
class SingleConv2dModule(torch.nn.Module):
    def __init__(self, ) -> None:
        ...
        self.maxpool2d = torch.nn.MaxPool2d(3, stride=2, padding=1)
        self.conv2 = nn.Conv2d(6, 18, (2, 2), stride=(1, 1), padding=(1, 1))

    def forward(self, x):
        temp = self.relu1(self.conv1(x))
        # return self.conv2(temp)
        
        # temp = self.relu1(x)
        # return temp
        temp_maxpool2d = self.maxpool2d(temp)

        return temp_maxpool2d
...
def test():
    ...
    for export_with_dynamic_shape in export_with_dynamic_shape_list:
        os.system("rm -rf /home/lesliefang/pytorch_1_7_1/inductor_quant/torch_script/inductor/int8/test_new_export/single_conv/torch_compile_debug/*")
