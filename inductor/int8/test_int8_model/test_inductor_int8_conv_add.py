...
def test_inductor_int8_conv_add():
    import copy
    ...
    class Mod(torch.nn.Module):
        def __init__(self, inplace_add=False) -> None:
            super().__init__()
        ...
        def forward(self, x):
            if not self.inplace_add:
                x1 = self.conv(x)
                return self.conv2(x1) + self.conv3(x1)
            else:
                x1 = self.conv(x)
                accum = self.conv2(x1)
                accum += self.conv3(x1)
...
def test_inductor_int8_conv_add_1_x():
    import copy
    from torch import _dynamo, _inductor
    from torch._inductor import config
    import logging
    ...
    m = quantize_fx.prepare_fx(m, qconfig_mapping, example_inputs, backend_config=backend_config)
    after_prepare_result = m(*example_inputs)
    print("model after prepare_pt2e is: {}".format(m), flush=True)
...
if __name__ == "__main__":
    test_inductor_int8_conv_add()
    #test_inductor_int8_conv_add_1_x()
