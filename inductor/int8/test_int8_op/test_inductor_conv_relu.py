...
def test_pytorch_op():
    with torch.no_grad():
        ...
        conv1 = model.conv1
        qconv = torch.ops.quantized.conv2d_relu.new
        ...
        res_ref = model(torch.dequantize(x_q))
...
def test_inductor_pytorch_op():
    with torch.no_grad():
        ...
        conv1 = qconv(
            ...
            None, strides, paddings, dilations, groups, torch.tensor(Y_scale), torch.tensor(Y_zero_point), "relu")
